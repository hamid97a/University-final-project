...
# *************************************
# *************************************
firstPage = session.get(url + "fa/law/search?page=" + str(page) + "&from_app_date=" +
                        changeDate(fromDate) + "&to_app_date=" + changeDate(toDate) + "&ot=d#aaa")
pageMax = int((firstPage.html.find(
    'div[id="myTabContent"]>ul>li', first=True).full_text.strip().split(":"))[1])
...
def dateSplit(string):
    if string != '':
        ...
        if s[2].startswith('0'):
            s[2] = s[2][1:]
        tarikh = str(JalaliDate(int(s[0]), int(
            s[1]), int(s[2])).to_gregorian())
        return tarikh
    else:
        return string
...
try:
    while page <= pageMax:
        try:
            lists = session.get(url + "fa/law/search?page=" + str(page) + "&from_app_date=" +
                                changeDate(fromDate) + "&to_app_date=" + changeDate(toDate) + "&ot=d#aaa")
        except:
            time.sleep(600)
            lists = session.get(url + "fa/law/search?page=" + str(page) + "&from_app_date=" +
        ...
        for each in trList:
            ...
            rootobj['announcementDate'] = dateSplit(
                tdList[3].full_text.strip())
    ...
    print("except")
    v = open("Error.txt", "a")
    v.write(" # Rules.py # In page : "+str(page) + " and id : " +
            str(lastId) + " we have except  "+str(datetime.now())+"\n")
    v.close()
